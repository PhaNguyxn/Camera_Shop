...
function ListCart(props) {
  ...
  const handlerDown = (getIdUser, getIdProduct, getCount) => {
    ...
    const currentCount = parseInt(getCount, 10);
              ...
              <tr className="text-center" key={value.idProduct || index}>
                ...
                <td className="align-middle border-0">
                  <p className="mb-0 small">
                    {(
                      Number(value.priceProduct.toString().replace(/\D/g, "")) *
                      Number(value.count)
                ...
                <td className="align-middle border-0">
                  <button
                    type="button"
                    className="reset-anchor remove_cart border-0 bg-transparent"
                    style={{
                      cursor: "pointer",
                    }}
                    onClick={() => handlerDelete(value.idUser, value.idProduct)}
                    aria-label={`Remove ${value.nameProduct || "product"} from cart`}
                  >
